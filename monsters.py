...
def capital(word): #helper function to allow capitalization ofonly the first char of a string without touching any other character.
...
def generate_dupilicates_list (monster_list, id): #monster_list is a list of monster data. id is a monster ID, Returns a list of monster ID's with duplicate names as the id, takes monster hacks into account.
    retval = []
    name = ID_To_String(getValue(id, 'id'))
    for it in range(0, len(monster_list)):
        if( ID_To_String(getValue(it, 'id')) == name and not "abstract" in data[it]):
            if('debug' in data[it]):
                if(data[it]['debug'] == 'true'):
                    break
            retval.append(it)
    return retval
...
def generatePage (var): #generates all the data which should be in one of the wiki pages. var is the position of the monster in the data master list of monsters.
    output = []
    
    if(ID_To_Mon_Int(data[var]['id']) == -1):
        return output
    if('abstract' in data[var]): #don't print abstracts
        return output
    if('debug' in data[var]): #dont print debug monsters
        if(data[var]['debug'] == 'true'):
            return output
    duplicates_list = generate_dupilicates_list(data, ID_To_Mon_Int(data[var]['id'])) #list of ID's with this name.
    
    for ite in range(0, len(duplicates_list)):
        id = duplicates_list[ite]
        if(ite == 0):
...
--><noinclude>""")
            if(len(duplicates_list) > 1): #dont show tabs if this is the only monster
                output.append("<div style=\"border-style: none;\">\n<tabs>\n")
        if(len(duplicates_list) > 1): #dont show tabs if this is the only monster
            output.append("<tab name=\""+capital(getValue(id,'name'))+" "+str(ite+1)+"\"style=\"border: 0px; padding: 0px; margin: 0px; border-style: none;\">\n")
        output.append("""{{enemydescription</noinclude>
...
<span class="plainlinks" style="font-size: small"><center>( [{{fullurl:{{lc:{{PAGENAME}}}}/doc|action=edit}} <span title="Edit user notes">Edit Notes</span>] )</center></span>
        ...
        if(len(duplicates_list) > 1): #Only print the closing of the tabs if there is more than one monster with this name.
            output.append("\n</tab>")
    if(len(duplicates_list) > 1): #Only print the closing of the tabs if there is more than one monster with this name.
        output.append("</tabs>\n</div>")
        ...
        if ( var == 'exit' ):
            ...
            for x in range(0, len(data)):
                ...
                if( text != []): #don't print an empty page
                    page = pywikibot.Page(site, capital(ID_To_String(data[x]['id']).split('|',1)[0]))
                    page.text = text
                    page.save('Updated text automatically via the https://github.com/Soyweiser/CDDA-Wiki-Scripts monsters.py script')
            var = raw_input(">")
        elif ( var == '_idtotal' ): #prints the max number of monsters
            print len(data)
            var = raw_input(">")
        else:
