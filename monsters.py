...
## accepts a mon_id value like mon_zombie, which returns the monster ID number, or a int which prints that the monster data of that number, or '_idtotal' which shows the total number of monsters (note the monster id numbers starts to count at 0, and the _idtotal starts counting at 1.
...
def generate_dupilicates_list (monster_list, id): #monster_list is a list of monster data. id is a monster ID, Returns a list of monster ID's with duplicate names as the id
    retval = []
    name = getValue(id,'name')
    for it in range(0, len(monster_list)):
        if( getValue(it,'name') == name and not "abstract" in data[it]):
            if('debug' in data[it]):
...
--><noinclude>""")
                ...
                if(len(duplicates_list) > 1): #dont show tabs if this is the only monster
                    output.append("<tab name=\""+getValue(id,'name').capitalize()+" "+str(ite+1)+"\"style=\"border: 0px; padding: 0px; margin: 0px; border-style: none;\">\n")
